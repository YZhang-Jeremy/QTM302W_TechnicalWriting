...
This data showcases aggregate YTD (Year to Date) data on arrests made by the NYPD in NYC. This data is extracted by the Office of Management Analysis and Planning and is manually tracted every quarter. The data includes information about the nature of the arrest, including location, time, and type of crime. Additionally, demographic information is also included, such as: race, age and sex. Our analysis will focus on one particular type of crime, namely shootings, and see how trends in shootings have changed over time in NYC. We will use the demographic information to see how these trends have affected different age, racial and sex groups.
More information and a link to our data can be found at https://catalog.data.gov/dataset/nypd-arrest-data-year-to-date. 
...
```
As stated before, we need to clean the variable PERP_AGE_GROUP to eliminate erroneous values like "940", "223" and "1020" because they do not correlate to age groups and are likely the result of incorrect manual inputs. We want to aggregate all of these inconsistancies, including entries like "UNKOWN" and "NULL" into the NA column.  
```{r}
# Check possible values
...
unique(cleaned_data$PERP_AGE_GROUP)
```
We will now inspect and clean PERP_SEX. For this particular cleaning, we are not only replacing "null" with NA, but also the "U" column. The "U" represents an "unspecified" entry, but since it has been misused historically, we will also aggregate it to the NA column. 
```{r}
# Check possible values
...
unique(cleaned_data$PERP_SEX)
```
Finally we will inspect and clean PERP_RACE. In order to make our graphs more legible, we will use recode() the names of our categories to ensure that they do not overlap on our visualizations
```{r}
# Check possible values
...
unique(cleaned_data$PERP_RACE)
```
Moving on from PERP data we will now inspect victim data starting with VIC_AGE_GROUP. Much of the cleaning will mirror what we did for PERP data, so reference those comments for any questions about why we are cleaning particular variables 
```{r}
# Check possible values
...
From Graph A we can see that the highest age group of victims of shootings are those between 25 and 44 years olds, followed by 18-24 year olds. Together these two age groups comprise of the majority of shooting cases addressed by the NYPD. From Graph B we see that the predominate racial group that falls victim to shootings are Black Americans. Finally graph C sheds some light into the historic trends of shootings and how it has fluctuated year to year. Interestingly it shows that in 2020 shootings skyrocketed and rose to previous levels seen around 2010. This might be explained by the effects that the COVID-19 pandemic and ensuing lockdown on the economy and society as a whole. 